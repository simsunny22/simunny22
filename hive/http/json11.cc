...
protected:
    ...
    const T m_value;
    void dump(string &out) const override { hive::dump(m_value, out); }
};
...
struct JsonParser {
	...
	// patch by pengjiang
	// hive use json int use uint64_value don't use int_value
        ...
        //}
       	...
       	if(str[i] != '.' && str[i] != 'e' && str[i] != 'E'
                && (i - start_pos) <= static_cast<uint64_t>(std::numeric_limits<uint64_t>::digits10)) {
            return std::strtoul(str.c_str() + start_pos, NULL, 0);
	}
	// patch end
