#include "stop_volume_driver_handler.hh"
...
static logging::logger logger("stop_volume_driver_handler");

sstring stop_volume_driver_handler::get_header_value(header_map& headers, sstring header_key, sstring default_value){
...
// public
// =============================================================================
future<std::unique_ptr<reply> > stop_volume_driver_handler::handle(
        const sstring& path
    ...
    sstring volume_ids = get_header_value(new_headers, "x-volume-id");
   
    auto volume_id_set = hive_tools::split_to_set(volume_ids, ":");

    std::vector<future<>> futs;
    for(auto& volume_id: volume_id_set){
        logger.error("stop_volume_driver_handler ........ volume_id:{}", volume_id);
        auto fut = stop_volume_driver(volume_id);
    ...
    return when_all(futs.begin(), futs.end()).then([rep = std::move(rep)](auto futs) mutable{
        try{
            ...
            return make_ready_future<std::unique_ptr<reply>>(std::move(rep));
             ...
             std::ostringstream out;
             out << "[stop_volume_driver_handler] error";
...
future<> stop_volume_driver_handler::stop_volume_driver(sstring volume_id){
    auto shard = get_local_volume_service().shard_of(volume_id);
    return get_volume_service().invoke_on(shard, [volume_id](auto& volume_service){
        //return make_ready_future<>();
        return volume_service.stop_volume_driver(volume_id);
    });
}
