#include "inject_extent_group_context_handler.hh"
...
static logging::logger logger("inject_extent_group_context_handler");
...
future<std::unique_ptr<reply> > inject_extent_group_context_handler::handle(const sstring& path,
    ...
    return seastar::async([context_json=std::move(context_json), rep=std::move(rep), context_ttl]()mutable {
        ...
        context_service.save_extent_group_context(context_json).get();
        rep->_content = "inject extent group context success";
        rep->done("success");
