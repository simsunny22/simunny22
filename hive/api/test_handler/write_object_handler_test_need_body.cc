#include "write_object_handler_test_need_body.hh"
...
static logging::logger logger("write_object_handler_test");

extern hive::hive_status hive_service_status;
extern sstring got_random(int range);
int64_t got_random_int(int64_t range){
    return rand() % range + 1;
}
...
void write_object_handler_test_need_body::check_write_command(hive_write_command& write_cmd){
...
hive_write_command write_object_handler_test_need_body::build_write_command(request& req){
    ...
    if (1024*1024 > length) {
        data_offset_in_extent   = got_random_int(1024*1024 - length); //tododl:yellow why only 0??
    }
...
future<std::unique_ptr<reply>> write_object_handler_test_need_body::handle(const sstring& path,
...
sstring write_object_handler_test_need_body::build_return_json(int64_t vclock){
