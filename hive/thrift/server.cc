...
static logging::logger logger("hive-thrift");
...
public:
    hive_thrift_stats(hive_thrift_server& server);
};

hive_thrift_server::hive_thrift_server()
        : _stats(new hive_thrift_stats(*this))
        , _handler_factory(create_hive_handler_factory().release())
        , _protocol_factory(new TBinaryProtocolFactoryT<TMemoryBuffer>())
        , _processor_factory(new HiveAsyncProcessorFactory(_handler_factory)) {
}
...
future<> hive_thrift_server::stop() {
    return make_ready_future<>();
}

struct handler_deleter {
    HiveCobSvIfFactory* hf;
    void operator()(HiveCobSvIf* h) const {
        hf->releaseHandler(h);
    }
};

class hive_thrift_server::connection {
...
public:
    connection(hive_thrift_server& server, connected_socket&& fd, socket_address addr)
    ...
    future<> process() {
//std::cout << "hvie thrift server process" << std::endl;
        return do_until([this] { return _read_buf.eof(); },
                [this] { return process_one_request(); });
    }
    future<> process_one_request() {
//std::cout << "hive thrfit server process request start" << std::endl;
        _input->resetBuffer();
        _output->resetBuffer();
        return read().then([this] {
//std::cout << "hive thrift server process_one_request" << std::endl;
    ...
    future<> read() {
//std::cout << "hive thrift server read start" << std::endl;
        return _read_buf.read_exactly(4).then([this] (temporary_buffer<char> size_buf) {
//std::cout << "hive thrift server read buf size: " << size_buf.size() << std::endl;
            if (size_buf.size() != 4) {
                return make_ready_future<>();
            }
            union {
            ...
            auto n = ntohl(data.n);
//std::cout << "hive thrift server read buf count: " << n << std::endl;
            return _read_buf.read_exactly(n).then([this, n] (temporary_buffer<char> buf) {
//std::cout << "hive thrift server read buf really count: " << buf.size() << std::endl;
                if (buf.size() != n) {
...
future<>
hive_thrift_server::listen(ipv4_addr addr, bool keepalive) {
    listen_options lo;
...
void
hive_thrift_server::do_accepts(int which, bool keepalive) {
    _listeners[which].accept().then([this, which, keepalive] (connected_socket fd, socket_address addr) mutable {
        fd.set_nodelay(true);
        fd.set_keepalive(keepalive);
//std::cout << "hive thrift do accepts addr: " << addr << std::endl;
...
uint64_t
hive_thrift_server::total_connections() const {
    return _total_connections;
}

uint64_t
hive_thrift_server::current_connections() const {
    return _current_connections;
}

uint64_t
hive_thrift_server::requests_served() const {
    return _requests_served;
}

hive_thrift_stats::hive_thrift_stats(hive_thrift_server& server)
        : _regs{
